...
現在の`縦`と`横`を将来の表示スタイルとしてどう表現するかは、コードを変更せず引き続き検討する。
すべてのスタイルがピアノロールであるため、選択肢へ`ピアノロール`を繰り返す名称は避けたい。
候補は次の2方式とし、公開名称を決めるまでは確定しない。

- `スタイル1`、`スタイル2`のような単純番号にする。
- 英語表記を含め、各方式の特徴を表す重複しない固有名称にする。
...
スタイルが生成した2D RGBA映像に対する最終的な位置、回転、拡大率等は利用者がAviUtl2側で調整する。
...
1. 鍵盤の色バリエーションを追加する。
   ...
   形状を決める`鍵盤タイプ`とは独立した設定とし、標準ピアノとハープ（7音）の両方で利用できる構成を検討する。
