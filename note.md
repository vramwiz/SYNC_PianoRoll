...
ノート矩形へ疑似的な立体感を加える表示も今後の検討対象とする。実際の3D座標へ変換するのではなく、
ノート色を基準に次の描画を重ねる。

- 外周へ暗い縁取りを付け、背景とノートの境界を明確にする。
- 光が当たる側の内縁を明るくし、反対側を暗くして浅い面取りに見せる。
- ノート表面の一部へ白またはノート色を明るくした半透明帯を重ね、つやを表現する。
- 必要であれば発音位置へ近い端を明るくし、時間方向を判別しやすくする。

設定は後から`ノート立体表示`等の単純なオン／オフとして追加し、通常利用向けの簡単設定へ置く。
縁の太さ、つやの幅、明暗量等を公開する必要が生じた場合だけ詳細設定へ追加する。
設定の意味と配色計算は全スタイルで共有するが、縦横の描画処理は共通化せず、それぞれの専用実装で
時間方向と音階方向に合う縁・つやの向きを決定する。発音時の鍵盤点灯と放射光を隠さない描画順にする。
...
終了時刻未満を対象とし、終了時刻が未確定のノートは0.2秒間点灯する。
