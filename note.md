...
現在実装済みの組み合わせは、ピアノ／2D／Type1の縦・横と、
ピアノ／3D／Type1の横である。3D Type1の縦は未実装のため、選択時は2D Type1の縦へ
安全にフォールバックする。今後専用実装を追加した時点で3Dの縦へ接続する。
...
回転する方式は採らず、音階をX軸、時間を奥行き方向とする平面をオブジェクト空間へ構築する。
形状自体には初期傾斜を持たせず、初期状態で2Dと同じ正面向きになるX-Y平面とする。
音階をX軸、時間をY軸、実形状の厚みをZ軸へ割り当てる。最終的な位置、カメラ角度、
回転、拡大率はAviUtl2側で調整する。
...
`DrawPoly`を利用できない環境では、描画不能にせず既存の縦表示へフォールバックする。
...
ピアノ／3D／Type1／横は実装済みで、AviUtl2実機でも現時点で最も良い状態まで確認した。
...
鍵盤の最新座標は、正面で黒鍵を白鍵のY範囲内へ短く重ねる。白鍵上面Zは固定し、
白鍵厚みを背面Zへ伸ばす。黒鍵は白鍵上面を底として手前Zへ立ち上げる。
白鍵とハープ鍵の内部境界には側面を作らず、鍵盤全体の左右外周と手前だけに側面を作る。
この状態のRelease版を`C:\ProgramData\aviutl2\Plugin\SYNC_PianoRoll`へ配置済みで、
全11テストと実DLL設定テストがPASSしている。

再開時は、まず現在の鍵盤を正面と複数のカメラ角度で再確認する。追加調整が不要なら、
次の候補は次のとおり。

1. 未実装のピアノ／3D／Type1／縦を専用実装する。現在は2D Type1縦へフォールバックする。
2. 3D Type1横へ発音中の鍵盤点灯・発光を追加するか検討する。
3. 円環／トンネル型または中央発音型を、ピアノ／3Dの次のTypeとして検討する。
