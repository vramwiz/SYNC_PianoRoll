...
今後はトラック番号ごとの色バリエーションと、MIDIキー（音階）ごとの色バリエーションを拡張する。
追加する配色要素は旧`Syncroh2_Filter_PianoRoll.dpr`が参照していた実装に合わせ、少なくとも次を
候補とする。
...
旧実装にはこのほか`ドレミ：虹／淡色／暗色`と`グラデ色：RGB／HSV`もある。これらを追加する場合も
新しい独自方式を先に増やさず、旧実装の要素、色計算、表示結果を基準にする。ただし旧クラス構成は
そのまま移植せず、現在の共通Colorユニットと縦横共通設定へ適合させて実装する。
