...
begin
  ...
  try
    ...
    if (string(PItemHeader(Items^[4])^.ItemType) <> 'select') or
      (string(PItemHeader(Items^[4])^.Name) <> '鍵盤タイプ') or
      (PSelectItem(Items^[4])^.Value <> 0) then
      raise Exception.Create('keyboard type item mismatch');
    if (string(PItemHeader(Items^[5])^.ItemType) <> 'track') or
      (string(PItemHeader(Items^[5])^.Name) <> '表示時間 (秒)') or
      (PTrackItem(Items^[5])^.Value <> 4.0) then
      raise Exception.Create('display time item mismatch');
    if string(PItemHeader(Items^[6])^.Name) <> '発音位置 (%)' then
      raise Exception.Create('strike position item mismatch');
    if string(PItemHeader(Items^[7])^.Name) <> 'ずらし (秒)' then
      raise Exception.Create('time shift item mismatch');
    if (string(PItemHeader(Items^[8])^.Name) <> '表示音階数') or
      (PTrackItem(Items^[8])^.Value <> 64) then
      raise Exception.Create('visible note count item mismatch');
    if (string(PItemHeader(Items^[9])^.Name) <> '中央ノート') or
      (PTrackItem(Items^[9])^.Value <> 64) then
      raise Exception.Create('center note item mismatch');
    if (string(PItemHeader(Items^[10])^.ItemType) <> 'select') or
      (string(PItemHeader(Items^[10])^.Name) <> '音域追従') then
      raise Exception.Create('pitch follow item mismatch');
    if string(PItemHeader(Items^[11])^.Name) <> '鍵盤の長さ' then
      raise Exception.Create('key length item mismatch');
    if PTrackItem(Items^[11])^.Value <> 120 then
      raise Exception.Create('key length default mismatch');
    if string(PItemHeader(Items^[12])^.Name) <> '鍵盤の太さ' then
      raise Exception.Create('key thickness item mismatch');
    if PTrackItem(Items^[12])^.Value <> 40 then
      raise Exception.Create('key thickness default mismatch');
    if string(PItemHeader(Items^[13])^.Name) <> 'ノート太さ (%)' then
      raise Exception.Create('note thickness item mismatch');
    if string(PItemHeader(Items^[14])^.Name) <> 'レーン表示' then
      raise Exception.Create('lane visibility item mismatch');
    if string(PItemHeader(Items^[15])^.Name) <> '拍線表示' then
      raise Exception.Create('beat visibility item mismatch');
    if string(PItemHeader(Items^[16])^.Name) <> '1小節の拍数' then
      raise Exception.Create('beats per measure item mismatch');
    if (string(PItemHeader(Items^[17])^.ItemType) <> 'color') or
      (string(PItemHeader(Items^[17])^.Name) <> '白鍵色') then
      raise Exception.Create('white key color item mismatch');
    if string(PItemHeader(Items^[18])^.Name) <> '黒鍵色' then
      raise Exception.Create('black key color item mismatch');
    if string(PItemHeader(Items^[19])^.Name) <> '白鍵レーン色' then
      raise Exception.Create('white lane color item mismatch');
    if string(PItemHeader(Items^[20])^.Name) <> '黒鍵レーン色' then
      raise Exception.Create('black lane color item mismatch');
    if string(PItemHeader(Items^[21])^.Name) <> '拍線色' then
      raise Exception.Create('beat line color item mismatch');
    if string(PItemHeader(Items^[22])^.Name) <> '小節線色' then
      raise Exception.Create('measure line color item mismatch');
    if string(PItemHeader(Items^[23])^.Name) <> '発音線色' then
      raise Exception.Create('strike line color item mismatch');
    if (string(PItemHeader(Items^[24])^.ItemType) <> 'select') or
      (string(PItemHeader(Items^[24])^.Name) <> 'ノート配色') or
      (PSelectItem(Items^[24])^.Value <> 0) then
      raise Exception.Create('track color mode item mismatch');
    if (string(PItemHeader(Items^[25])^.ItemType) <> 'color') or
      (string(PItemHeader(Items^[25])^.Name) <> 'ノート単色') then
      raise Exception.Create('single track color item mismatch');
    if (string(PItemHeader(Items^[26])^.ItemType) <> 'select') or
      (string(PItemHeader(Items^[26])^.Name) <> 'ノート立体表示') or
      (PSelectItem(Items^[26])^.Value <> 1) then
      raise Exception.Create('note depth item mismatch');
    if (string(PItemHeader(Items^[27])^.ItemType) <> 'color') or
      (string(PItemHeader(Items^[27])^.Name) <> 'グラデ色1') or
      (PColorItem(Items^[27])^.R <> 255) or
      (PColorItem(Items^[27])^.G <> 0) or
      (PColorItem(Items^[27])^.B <> 0) then
      raise Exception.Create('gradient color 1 item mismatch');
    if (string(PItemHeader(Items^[28])^.ItemType) <> 'color') or
      (string(PItemHeader(Items^[28])^.Name) <> 'グラデ色2') or
      (PColorItem(Items^[28])^.R <> 0) or
      (PColorItem(Items^[28])^.G <> 0) or
      (PColorItem(Items^[28])^.B <> 255) then
      raise Exception.Create('gradient color 2 item mismatch');
    if (PColorItem(Items^[17])^.R <> 242) or
      (PColorItem(Items^[17])^.G <> 242) or
      (PColorItem(Items^[17])^.B <> 242) or
      (PColorItem(Items^[17])^.X <> 0) then
      raise Exception.Create('white key default color mismatch');
    if (PColorItem(Items^[22])^.R <> 255) or
      (PColorItem(Items^[22])^.G <> 190) or
      (PColorItem(Items^[22])^.B <> 80) or
      (PColorItem(Items^[22])^.X <> 0) then
      raise Exception.Create('measure line default color mismatch');
    if (PColorItem(Items^[25])^.R <> 80) or
      (PColorItem(Items^[25])^.G <> 210) or
      (PColorItem(Items^[25])^.B <> 255) or
      (PColorItem(Items^[25])^.X <> 0) then
      raise Exception.Create('single track default color mismatch');

    if (PSelectItem(Items^[4])^.List = nil) or
      (string(PSelectListArray(
        PSelectItem(Items^[4])^.List)^[0].Name) <> '標準ピアノ') or
      (string(PSelectListArray(
        PSelectItem(Items^[4])^.List)^[1].Name) <> 'ハープ（7音）') or
      (PSelectListArray(PSelectItem(Items^[4])^.List)^[2].Name <> nil) then
      raise Exception.Create('keyboard type select list mismatch');
    if (PSelectItem(Items^[10])^.List = nil) or
      (string(PPitchFollowListArray(
        PSelectItem(Items^[10])^.List)^[0].Name) <> '追従しない') or
      (string(PPitchFollowListArray(
        PSelectItem(Items^[10])^.List)^[1].Name) <> '常に追従') or
      (string(PPitchFollowListArray(
        PSelectItem(Items^[10])^.List)^[2].Name) <>
        'はみ出したとき追従') or
      (PPitchFollowListArray(
        PSelectItem(Items^[10])^.List)^[3].Name <> nil) then
      raise Exception.Create('pitch follow select list mismatch');
    if (PSelectItem(Items^[14])^.List = nil) or
      (PSelectListArray(PSelectItem(Items^[14])^.List)^[2].Name <> nil) then
      raise Exception.Create('visibility select list is not terminated');
    if (PSelectItem(Items^[24])^.List = nil) or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[0].Name) <>
        '単色') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[1].Name) <>
        'バリエーション1') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[2].Name) <> 'トラック：DOS') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[5].Name) <> 'トラック：暗色') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[6].Name) <> '音階：DOS') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[9].Name) <> '音階：暗色') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[10].Name) <> 'ドレミ：虹') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[12].Name) <> 'ドレミ：暗色') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[13].Name) <> 'グラデ色：RGB') or
      (string(PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[14].Name) <> 'グラデ色：HSV') or
      (PColorModeListArray(
        PSelectItem(Items^[24])^.List)^[15].Name <> nil) then
    ...
    if (PSelectItem(Items^[26])^.List = nil) or
      (string(PSelectListArray(
        PSelectItem(Items^[26])^.List)^[0].Name) <> '平面') or
      (string(PSelectListArray(
        PSelectItem(Items^[26])^.List)^[1].Name) <> '立体') or
      (PSelectListArray(
        PSelectItem(Items^[26])^.List)^[2].Name <> nil) then
